	...
	uint64_t nanos, PyObject* py_policy, as_error* err)
{
	...
	status = aerospike_truncate(self->as, err, info_policy_p, namespace, set, nanos);
...
AerospikeClient_Truncate(AerospikeClient * self, PyObject * args, PyObject * kwds)
{
	...
	if (PyString_Check(py_ns)) {
		...
		if (!namespace) {
			as_error_update(&err, AEROSPIKE_ERR_CLIENT, "Memory allocation failed");
			goto CLEANUP;
		}
		...
		as_error_update(&err, AEROSPIKE_ERR_PARAM, "Namespace must be unicode or string type");
		goto CLEANUP;
	}
	...
	if (PyString_Check(py_set)) {
		...
		as_error_update(&err, AEROSPIKE_ERR_PARAM, "Set must be None, or unicode or string type");
		goto CLEANUP;
	}
	...
	if (PyLong_Check(py_nanos)) {
		...
		if (temp_long < 0 && !PyErr_Occurred()) {
			as_error_update(&err, AEROSPIKE_ERR_PARAM, "Nanoseconds must be a positive value");
			goto CLEANUP;
		}
		...
		if (PyErr_Occurred()) {
			as_error_update(&err, AEROSPIKE_ERR_PARAM, "Nanoseconds value too large");
			goto CLEANUP;
		}
		...
		if (tempInt == -1 && PyErr_Occurred()) {
			as_error_update(&err, AEROSPIKE_ERR_PARAM,"Nanoseconds value out of range for long");
			goto CLEANUP;
		}

		if (tempInt < 0) {
			as_error_update(&err, AEROSPIKE_ERR_PARAM, "Nanoseconds value must be a positive value");
			goto CLEANUP;
		}
		...
		as_error_update(&err, AEROSPIKE_ERR_PARAM, "Nanoseconds must be a long type");
		goto CLEANUP;
	}

	ret_val = AerospikeClient_TruncateInvoke(self, namespace, set, nanos, py_policy, &err);
